	...
	*/
	...
	$.ajax({
		  ...
		  datatype: "json"
			...
			console.log ( 'Procesando fichero principal: datos.json' );
			console.log (resultado);

//			var lista_ficheros = JSON.parse ( resultado );
			var lista_ficheros = resultado;
			var json_completo = Array ();

			for ( i = 0 ; i < lista_ficheros.length ; i++ ) {
				var n_objeto = procesar_fichero_json ( lista_ficheros[i] )
				console.log ( 'Push:' );
				console.log ( n_objeto );
				if ( i == 0 ) {
					json_completo.push ( n_objeto );
				} else {
					for ( fila in n_objeto ) {
						console.log ( n_objeto[fila] );
						json_completo.push ( n_objeto[fila] );
					}
				}
			}
...
function procesar_fichero_json ( fichero_json )
{
	var objeto_devuelto;
	$.ajax({
		  url: fichero_json,
		  ...
		  datatype: "json"
			...
			console.log ( 'Procesando fichero: '+fichero_json );
			console.log (resultado );
			objeto_devuelto = resultado;
		});	
	return objeto_devuelto;
}


function procesar_json ( resultado )
...
//		cuerpo += '<td><a href="#ficha_'+resultado[i]['2']+'" data-rel="dialog" data-transition="pop">'+resultado[i]['3']+'</a></td>';
	...
	//$('table#resultados').table( "refresh" ).trigger( "create" );
